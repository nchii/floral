<?php
    ...
    function create_table($mysqli){
        ...
        $sql = "CREATE TABLE IF NOT EXISTS `bouquet`(`id` INT AUTO_INCREMENT,`name` VARCHAR(45) NOT NULL, `description` VARCHAR(225) NOT NULL,`price` INT NOT NULL,`img` VARCHAR(255) NOT NULL,PRIMARY KEY(`id`))";
        if(!$mysqli->query($sql)){
            return false;
        }
        ...
        $sql = "CREATE TABLE IF NOT EXISTS `invoice`(`id` INT AUTO_INCREMENT,`user_id` INT NOT NULL,`total_amount` INT NOT NULL,`date` DATETIME NOT NULL,PRIMARY KEY(`id`),FOREIGN KEY (`user_id`) REFERENCES `user`(`id`) )";
        if(!$mysqli->query($sql)){
            return false;
        }
